...
class TrainDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        MixData_out = torch.unsqueeze(MixData[-5:, :, :], 0)  # the last five frame 在第 0 维插入一个新轴，长度为 1
        MixData_out = MixData_out.repeat(3, 1, 1, 1)#会把张量在第0维重复3次、其余维度不变，

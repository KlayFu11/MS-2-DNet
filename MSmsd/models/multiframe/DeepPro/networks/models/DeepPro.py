...
from deepmist.models.multiframe.DeepPro.DeepPro.networks.layers.basic import TD_Resblock, STD_Resblock
from deepmist.models.multiframe.DeepPro.DeepPro.networks.layers.basic import TDifferenceConv, SDifferenceConv
from deepmist.models.multiframe.DeepPro.DeepPro.networks.layers.TPro import TPro
from deepmist.models.multiframe.DeepPro.DeepPro.networks.losses.HAM_loss_MultiFrame import HAM_loss
from deepmist.models.multiframe.DeepPro.DeepPro.networks.losses.HPM_loss_MultiFrame import HPM_loss
...
class detector(nn.Module):
    def __init__(self, num_classes, seqlen=100, out_len=100):
        ...
        self.conv_in = nn.Sequential(nn.Conv3d(in_channels=3, out_channels=8, kernel_size=(5,1,1), stride=(1,1,1), padding=(2,0,0)),
...
if __name__ == '__main__':
    # 创建模型实例，设置seqlen参数为5（与输入帧数一致）
    model = detector(num_classes=1, seqlen=5, out_len=5).cuda()
    # 创建输入张量：[batch_size, channels, seq_len, height, width]
    inputs = torch.randn((1, 3, 5, 384, 384)).cuda()
    # 使用thop库分析模型参数量和FLOPs

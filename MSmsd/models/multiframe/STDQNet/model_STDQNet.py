...
from deepmist.models.multiframe.STDQNet import base_STD as base
...
class CHUNK(nn.Module):
    def __init__(self, num_feat):
        ...
        chunk_channels = num_feat // 2
        pw_channels = num_feat * 2

        # 四个不同卷积核大小的并行卷积
        self.conv1 = nn.Conv2d(chunk_channels, chunk_channels, kernel_size=1, padding=0)
        self.conv3 = nn.Conv2d(chunk_channels, chunk_channels, kernel_size=3, padding=1)
        self.conv5 = nn.Conv2d(chunk_channels, chunk_channels, kernel_size=5, padding=2)
        self.conv7 = nn.Conv2d(chunk_channels, chunk_channels, kernel_size=7, padding=3)

        self.PWconv = nn.Conv2d(pw_channels, pw_channels, kernel_size=1, padding=0)

        # 最终融合卷积：将 4 * chunk 通道 → num_feat
        self.fusion = nn.Conv2d(2 * num_feat, num_feat, kernel_size=3, padding=1)
    ...
    def forward(self, past_feat, curr_feat):
        ...
        # 四种卷积
        y0 = self.act(self.conv1(x0))
        y1 = self.act(self.conv3(x1))
        y2 = self.act(self.conv5(x2))
        y3 = self.act(self.conv7(x3))

        # 再次通道堆叠
        y = torch.cat([y0, y1, y2, y3], dim=1)  # [B, 2C, H, W]

        #y = self.act(self.PWconv(y))
        y = self.PWconv(y)
        # 输出一个和原通道数一致的 observation
        out = self.fusion(y)  # [B, C, H, W]
...
class STDQNet(nn.Module):
    def __init__(self, num_frames=5, num_classes=1, in_channels=3, block=base.ResBlock, num_blocks=[2, 2, 2],
        ...
        self.CHK = nn.ModuleList([
            CHUNK(num_feat=nb_filter[j]) for j in range(self.num_levels)
        ])
    ...
    def forward(self, x):
        ...
        for i in range(self.num_levels):
            for j in range(self.num_frames):
                all_compensated_feats[i].append(self.CHK[i](all_level_feats[i][j], all_level_feats[i][-1]))

...
"""
对于每个像素，它查看自己周围 3x3 的邻居，找到对应的特征值 x 和对应的动态权重 w，把它们乘起来加在一起，这就完成了“Focus Small”的局部聚合。
"""

@triton.jit
def ska_fwd(
    x_ptr, w_ptr, o_ptr,#输入数据的显存指针，特征，权重，输出
    n, ic, h, w, ks, pad, wc,#维度信息，batch，channels，height，width，ks，padding,weight_channels
    BS: tl.constexpr,  #Block Size,每个Triton程序块处理的数据量
    CT: tl.constexpr, AT: tl.constexpr  #数据类型
):
    pid = tl.program_id(0)#获取当前id
    start = pid * BS#计算起始偏移量
    #计算当前程序块处理的偏移量，告诉并行线程，当前计算输出的是第offs个位置的值
    offs = start + tl.arange(0, BS)


    #多维坐标计算，_idx将一维坐标转化为多维
    ni, ci, hi, wi, m = _idx(offs, n, ic, h, w)
    val = tl.zeros((BS,), dtype=AT)#初始化累加器

#遍历kernel_size即小核卷积的宽高
    for kh in range(ks):
        hin = hi - pad + kh
        hb = (hin >= 0) & (hin < h)#防止y坐标超出范围
        for kw in range(ks):
            win = wi - pad + kw
            b = hb & (win >= 0) & (win < w)#防止X坐标超出范围

            #把多维坐标转变为一维地址从而寻找x以及周围的ks*ks个像素值
            x_off = ((ni * ic + ci) * h + hin) * w + win
            #ci%wc 实现将输入通道按权重通道分组，每个组使用同一个动态权重
...
#SKaFn继承至torch.autograd.Function类，提供apply方法，调用apply触发forward执行前向计算，是调用自定义自动微分函数的标准方式
#将TRITO内核能像Pytorch函数一样被调用进行的封装
#@custom_fwd 和 @custom_bwd 装饰器支持混合精度计算
class SkaFn(Function):
    @staticmethod
    @custom_fwd(device_type='cuda')
    def forward(ctx, x: torch.Tensor, w: torch.Tensor) -> torch.Tensor:
        ks = int(math.sqrt(w.shape[2]))#ks大小计算
        pad = (ks - 1) // 2
        ctx.ks, ctx.pad = ks, pad
        n, ic, h, width = x.shape
        wc = w.shape[1]#分组值
        #申请输出张量的显存，创建一个空的Tensor用于存放输出
        ...
        # 启动内核时，PyTorch 会自动把 Tensor o 转换为它的底层数据指针 o_ptr 传给 kernel
...
# 输入：x ∈ [N, Cin, H, W]
# 权重：w ∈ [N, G, K², H, W]  (group dynamic kernels)
# 输出：o ∈ [N, Cin, H, W]   # 每个输入通道一个输出通道


#接收梯度go,(Gradient Output)
    @staticmethod

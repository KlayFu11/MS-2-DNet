...
from . import Deform_part as deform
...
class ResBlock_3d(nn.Module):
    def __init__(self, nf):
        ...
        #DeformConvpack_d即为D3D
        self.dcn0 = deform.DeformConvPack_d(nf, nf, kernel_size=3, stride=1, padding=1, dimension='HW')
        self.dcn1 = deform.DeformConvPack_d(nf, nf, kernel_size=3, stride=1, padding=1, dimension='HW')
        self.lrelu = nn.LeakyReLU(negative_slope=0.1, inplace=False)
    def forward(self, x):
        # return self.dcn1(self.lrelu(self.dcn0(x))) + x
        if torch.isnan(x).any() or torch.isinf(x).any():
            print("Warning: NaN/Inf detected in ResBlock_3d input")
        out = self.dcn0(x)
        out = self.lrelu(out)
        out = self.dcn1(out)
        # 确保输出是有效的
        if torch.isnan(out).any() or torch.isinf(out).any():
            print("Warning: NaN/Inf detected in ResBlock_3d output")
        return out + x
...
class STDNet(nn.Module):
    ...
    def make_layer(self, block1, num_of_layer):
        layers = []
    ...
    def forward(self, x):
        ...
        for i in range(self.num_frames):#0,1,2,3,4
            #past_frame_feats是0,1,2,3前四帧的4个特征
            # past_frame_feats = curr_frame_feats if i == self.num_frames - 1 else self.encoder(x[:, :, i, :, :])
            #ImplicitMotionCompensation(ResNet, [8,16,32,64], 4, [3, 3, 3, 3], [3, 5, 7])
            ...
            for j in range(self.num_levels):#0,1,2,3
                #4个层级每个层级分别生成四个all_compensated_feats,先生成当前帧和第一帧的，然后是当前帧和第二帧的·····
                #即在[[], [], [], []]中按x轴添加，即[[1], [2], [3], [4]]，[[1，5], [2，6], [3，7], [4，8]]······
                all_level_feats[j].append(frame_feats[j])

        all_5D_level_feats = []
        for j in range(self.num_levels):
            merge_feat = torch.stack(all_level_feats[j], dim=2)#(1,c,5,h,w)
            all_5D_level_feats.append(merge_feat)

        #Deformable 3D Convolution
        observations = []
        for j in range(self.num_levels):
            b, c, t, h, w = all_5D_level_feats[j].shape
            feat_residual = self.residual_layer[j](all_5D_level_feats[j])
            feat_D3D = self.TA[j](feat_residual.permute(0, 2, 1, 3, 4).contiguous().view(b, -1, h, w))
            observations.append(feat_D3D)

...
class DTUM(nn.Module):  # final version
    def __init__(self, in_channels, num_classes, num_frames):
        ...
        self.final = nn.Sequential(
            nn.Conv3d(in_channels=2 * inch, out_channels=32, kernel_size=(1, 3, 3), stride=(1, 1, 1),
                      padding=(0, 1, 1)),
            nn.BatchNorm3d(32), nn.ReLU(),
            nn.Dropout3d(0.5),
            nn.Conv3d(in_channels=32, out_channels=num_classes, kernel_size=(1, 1, 1), stride=(1, 1, 1),
                      padding=(0, 0, 0)),
        )
    ...
    def forward(self, x):
        ...
        return x_out, x_4.reshape(1, 32 * 5, 48, 48), o_3.reshape(1, 32 * 5, 96, 96), o_2.reshape(1, 32 * 5, 192, 192), o_1.reshape(1, 32 * 1, 384, 384)

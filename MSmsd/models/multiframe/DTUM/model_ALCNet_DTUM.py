...
from deepmist.models.singleframe.ACM.fusion import AsymBiChaFuse
...
class ASKCResNetFPN(nn.Module):
    def __init__(self, in_channels=3, layers=[4, 4, 4, 4], channels=[8, 16, 32, 64, 128], fuse_mode='AsymBi',
                 act_dilation=16, classes=1, tinyFlag=False, norm_layer=BatchNorm2d, groups=1, norm_kwargs=None,
                 **kwargs):
        ...
        if tinyFlag:
            ...
            self.stem = nn.Sequential(
                ...
                nn.Conv2d(in_channels=in_channels, out_channels=stem_width, kernel_size=3, stride=2, padding=1,
                          bias=False),
            ...
            self.head = _FCNHead(in_channels=2 * channels[0], channels=classes,
                                 momentum=self.momentum)  ###########in_channels###########
            ...
            self.deconv0 = nn.ConvTranspose2d(in_channels=channels[1], out_channels=2 * channels[0], kernel_size=(4, 4),
                                              ###########out_channels###########
                                              stride=2, padding=1)
...
class ALCNet_DTUM(nn.Module):
    def __init__(self, in_channels=3, layers=[4, 4, 4, 4], channels=[8, 16, 32, 64, 128], fuse_mode='AsymBi',
        ...
        self.ALCNet = ASKCResNetFPN(in_channels=in_channels, layers=layers, channels=channels, fuse_mode=fuse_mode,
                                    act_dilation=act_dilation, classes=32, tinyFlag=tinyFlag, norm_layer=norm_layer,
                                    groups=groups, norm_kwargs=norm_kwargs)
    ...
    def forward(self, X_In):
        ...
        Features = self.ALCNet(Features)  # (B, 32, H, W)
...
if __name__ == '__main__':
    ...
    model = ALCNet_DTUM(input_channels=3).to(device)
    inputs = torch.randn((1, 3, 5, 384, 384)).to(device)  # Params = 0.84M FLOPs = 17.28G

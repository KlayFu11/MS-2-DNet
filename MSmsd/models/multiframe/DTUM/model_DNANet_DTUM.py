...
class DNANet(nn.Module):
    ...
    def forward(self, input):
        x0_0 = self.conv0_0(input)  # 512
        x1_0 = self.conv1_0(self.pool(x0_0))  # 256
        x0_1 = self.conv0_1(torch.cat([x0_0, self.up(x1_0)], 1))  # 512

        x2_0 = self.conv2_0(self.pool(x1_0))  # 128
        x1_1 = self.conv1_1(torch.cat([x1_0, self.up(x2_0), self.down(x0_1)], 1))  # 256
        x0_2 = self.conv0_2(torch.cat([x0_0, x0_1, self.up(x1_1)], 1))  # 512

        x3_0 = self.conv3_0(self.pool(x2_0))  # 64
        x2_1 = self.conv2_1(torch.cat([x2_0, self.up(x3_0), self.down(x1_1)], 1))  # 128
        x1_2 = self.conv1_2(torch.cat([x1_0, x1_1, self.up(x2_1), self.down(x0_2)], 1))  # 256
        x0_3 = self.conv0_3(torch.cat([x0_0, x0_1, x0_2, self.up(x1_2)], 1))  # 512
...
class DNANet_DTUM(nn.Module):
    ...
    def forward(self, X_In):
        ...
        if self.deep_supervision:
            Features, x_out_0, x_out_1 = self.DNANet(Features)
        else:
            Features = self.DNANet(Features)
        ...
        if self.deep_supervision:
            return [X_Out, x_out_0, x_out_1]
        else:
            return X_Out


if __name__ == '__main__':
    # model = DNANet_DTUM(input_channels=1).cuda()
    # inputs = torch.randn((4, 1, 5, 256, 256)).cuda()  # Params = 1.2M FLOPs = 177.32G

    model = DNANet_DTUM(input_channels=3).cuda()
    inputs = torch.randn((1, 3, 5, 384, 384)).cuda()  # Params = 1.21M FLOPs = 99.98G

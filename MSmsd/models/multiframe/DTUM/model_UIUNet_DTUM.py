...
from deepmist.models.singleframe.UIUNet.fusion import *
...
class UIUNet(nn.Module):
    ...
    def __init__(self, in_ch=3, out_ch=1, deep_supervision=False):
        ...
        if deep_supervision:
            self.side1_1 = nn.Conv2d(out_ch, 1, 3, padding=1)
            self.side2_1 = nn.Conv2d(out_ch, 1, 3, padding=1)
            self.side3_1 = nn.Conv2d(out_ch, 1, 3, padding=1)
            self.side4_1 = nn.Conv2d(out_ch, 1, 3, padding=1)
            self.side5_1 = nn.Conv2d(out_ch, 1, 3, padding=1)
            self.side6_1 = nn.Conv2d(out_ch, 1, 3, padding=1)
...
class UIUNet_DTUM(nn.Module):
    ...
    def forward(self, X_In):
        ...
        if self.deep_supervision:
            return [X_Out, d1, d2, d3, d4, d5, d6]
        else:
            return X_Out


if __name__ == '__main__':
    # model = UIUNet_DTUM(in_ch=1).cuda()
    # inputs = torch.randn((4, 1, 5, 256, 256)).cuda()  # Params = 51.05M FLOPs = 1152.31G

    model = UIUNet_DTUM(in_ch=3).cuda()
    inputs = torch.randn((1, 3, 5, 384, 384)).cuda()  # Params = 51.05M FLOPs = 649.02G

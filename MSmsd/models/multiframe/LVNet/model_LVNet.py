...
class LVNet(nn.Module):
    """
    Args:
        num_frame (int): Number of frames of the input image. Default: 5.
    ...
    def __init__(self,
        ...
        #self.cnnEncoderL1 = CNNEncoder(3, embed_dim // 4)
        ...
        for i_layer in range(self.num_layers):
            ...
            layer = BasicLayer(
                dim=int(embed_dim * 2 ** (self.num_layers - 1 - i_layer)) if i_layer > 0 else embed_dim * 2 ** (
                        self.num_layers - 1),
    ...
    def forward(self, x):
        ...
        cnnShortCut = []
        x = self.cnnEncoderL1(x)
        cnnShortCut.append(x)
        x = self.cnnDownL1(x)
        x = self.cnnEncoderL2(x)
        cnnShortCut.append(x)
        x = self.cnnDownL2(x)
        x = self.cnnEncoderL3(x)
        x = rearrange(x, '(b d) c h w -> b c d h w', d=self.num_frame)
        ...
        x, _ = self.layers_up[0](x)

        for layer, linear in zip(self.layers_up[1:], self.layers_ct[1:]):
...
if __name__ == '__main__':
    model = LVNet().cuda()
    inputs = torch.randn((1, 3, 5, 384, 384)).cuda()  # Params = 1.77M FLOPs = 13.93G

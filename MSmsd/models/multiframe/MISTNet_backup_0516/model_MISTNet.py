...
# from deepmist.models.multiframe.MISTNet_backup_0516.base import *
...
class ImplicitMotionCompensation(nn.Module):
    ...
    def forward(self, past_frame_feats, curr_frame_feats):
        return [self.SSCA_list[j](past_frame_feats[j], curr_frame_feats[j]) for j in range(self.num_levels)]
...
class MultiFrameAggregation(nn.Module):
    ...
    def forward(self, compensated_feats):
        return [self.inter_frame_fusion_list[j](compensated_feats[j]) for j in range(self.num_levels)]
...
class SelfDistillation(nn.Module):
    ...
    def forward(self, v):
        ...
        z = self.bottleneck(v)  # Variational Information Bottleneck: v -> z
...
class ProgressiveDistillationDecoder(nn.Module):
    def __init__(self, num_classes, block, nb_filter, modulator):
        ...
        self.decoder_2 = SelfDistillation(block, nb_filter[2] + nb_filter[3], nb_filter[2], num_classes, modulator)
        self.decoder_1 = SelfDistillation(block, nb_filter[1] + nb_filter[2], nb_filter[1], num_classes, modulator)
        self.decoder_0 = SelfDistillation(block, nb_filter[0] + nb_filter[1], nb_filter[0], num_classes, modulator)

    def forward(self, observations):
        z_3, pred_z_3, pred_v_3 = self.decoder_2(torch.cat([observations[2], self.up(observations[3])], 1))  # [96, 96]
        pred_z_3, pred_v_3 = self.up_4(pred_z_3), self.up_4(pred_v_3)  # [384, 384]
        z_2, pred_z_2, pred_v_2 = self.decoder_1(torch.cat([observations[1], self.up(z_3)], 1))  # [192, 192]
        pred_z_2, pred_v_2 = self.up(pred_z_2), self.up(pred_v_2)  # [384, 384]
        z_1, pred_z_1, pred_v_1 = self.decoder_0(torch.cat([observations[0], self.up(z_2)], 1))  # [384, 384]
...
class MISTNet(nn.Module):
    def __init__(self, num_frames=5, num_classes=1, in_channels=3, block=ResBlock, num_blocks=[2, 2, 2],
        ...
        self.SSCP = ImplicitMotionCompensation(block, nb_filter, self.num_levels, shift_sizes, neighborhood_sizes)
        self.IFF = MultiFrameAggregation(block, nb_filter, num_frames, self.num_levels)
    ...
    def forward(self, x):
        ...
        for i in range(self.num_frames):
            ...
            compensated_feats = self.SSCP(past_frame_feats, curr_frame_feats)
            for j in range(self.num_levels):
                all_compensated_feats[j].append(compensated_feats[j])

        # Multi-frame Aggregation
        observations = self.IFF([torch.cat(feats, dim=1) for feats in all_compensated_feats])

...
class Down(nn.Module):
    def __init__(self,
        ...
        self.stem = nn.Sequential(
            # normLayer(inchannels, affine=False),
            # normLayer(inchannels),
            nn.Conv2d(inchannels, stemWidth * 2, kernel_size=3, stride=1, padding=1, bias=False),
...
class ISTUDNet(miNet):
    def __init__(self, input_channels=1, classes=1):
        super(ISTUDNet, self).__init__()
        self.encoder = None
        self.decoder = None
        ...
        self.down = Down(inchannels=input_channels, channels=[16, 32, 64, 128])
        # self.up = lambda x:x
        ...
        self.headSeg = Head(inpChannel=64, oupChannel=classes)
        # self.DN = DN()
    ...
    def funOutput(self, x):
        ...
        return self.headSeg(x)  # torch.sigmoid(self.headDet(x)),

...
class CrossScaleAttention(nn.Module):
    def __init__(self, dim, img_shape=576, att_shape=784):  # 784   img_shape=2304, att_shape=3136
        # def __init__(self, dim, img_shape=2304, att_shape=3136):
        super().__init__()
    ...
    def forward(self, x):
        ...
        attn = attn.permute(0, 2, 1).contiguous().view(x0.shape[0], x0.shape[1], 24, 24)  # 24
        # attn = attn.permute(0, 2, 1).contiguous().view(x0.shape[0], x0.shape[1], 48, 48)
...
class CIM(nn.Module):
    def __init__(self, dim, num_layers=1, channels=[8, 16, 32, 64], downsample=2):  # [8, 16, 32, 64]
        # def __init__(self, dim, num_layers=1, channels=[8, 16, 32, 64], downsample=1):
        super().__init__()
        ...
        self.block = CIMBlock(self.hidden_dim, channels)
    ...
    def forward(self, input):  # [B, C, H, W]
        # down sample features and concat
        out = self.pool(input)
        # squeeze channels
        ...
        # fusion encoder feature and global feature
        results = []
...
if __name__ == '__main__':
    model = CIM(120)
    x1 = torch.randn(1, 8, 384, 384)
    x2 = torch.randn(1, 16, 192, 192)
    x3 = torch.randn(1, 32, 96, 96)
    x4 = torch.randn(1, 64, 48, 48)
    x = tuple([x1, x2, x3, x4])
    y = model(x)
    a = 1

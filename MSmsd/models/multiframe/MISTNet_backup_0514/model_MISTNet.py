...
class ResNet(nn.Module):
    def __init__(self, in_channels, block, num_blocks, nb_filter):
        ...
        self.encoder_4 = make_layer(block, nb_filter[3], nb_filter[4], num_blocks[3])  # 4
    ...
    def forward(self, x):
        ...
        x_e4 = self.encoder_4(self.pool(x_e3))  # [1, 128, 24, 24]

        return x_e0, x_e1, x_e2, x_e3, x_e4
...
class SpatialShiftCorrAggr(nn.Module):
    def __init__(self, block, in_channels, shift_size=3, neighbor_sizes=[3, 5, 7], dilation_rates=[1, 1, 1]):
        ...
        self.conv_fusion = make_layer(block, self.in_channels * (self.num_levels + 1), self.in_channels, num_blocks=1)  # 2
...
class SpatialShiftTempCorrPyramid(nn.Module):
    def __init__(self, block, nb_filter, num_stages, shift_sizes=[3, 3, 3, 0, 0]):
        super(SpatialShiftTempCorrPyramid, self).__init__()
...
class ProgressiveDistillationDecoder(nn.Module):
    def __init__(self, num_classes, block, nb_filter, ratio=16, activation='CBAM'):
        ...
        self.decoder_3 = VSD(block, nb_filter[3] + nb_filter[4], nb_filter[3], num_classes, ratio, activation)
    ...
    def forward(self, x_e0, x_e1, x_e2, x_e3, x_e4):
        # stage 3
        z_d3, pred_z_d3, pred_v_d3 = self.decoder_3(torch.cat([x_e3, self.up(x_e4)], 1))  # [48, 48]
        pred_z_d3, pred_v_d3 = self.up_8(pred_z_d3), self.up_8(pred_v_d3)  # [384, 384]
        # stage 2
        z_d2, pred_z_d2, pred_v_d2 = self.decoder_2(torch.cat([x_e2, self.up(z_d3)], 1))  # [96, 96]
        ...
        pred = pred_z_d0
        pred_z_list = [pred_z_d0, pred_z_d1, pred_z_d2, pred_z_d3]
        pred_v_list = [pred_v_d0, pred_v_d1, pred_v_d2, pred_v_d3]
...
class MISTNet(nn.Module):
    def __init__(self, num_inputs=5, num_classes=1, in_channels=3, block=Res_block, num_blocks=[2, 2, 2, 2],
                 nb_filter=[8, 16, 32, 64, 128], num_stages=5, use_ib_loss=True, deep_supervision=False):
        ...
        self.SSTCP = SpatialShiftTempCorrPyramid(block, nb_filter, num_stages, shift_sizes=[3, 3, 3, 3, 3])
    ...
    def forward(self, x):
        ...
        for i in range(self.num_inputs):
            ...
            all_frames_aggregated_x_e4.append(aggregated_x_e[4])
        ...
        all_frames_aggregated_x_e4 = torch.cat(all_frames_aggregated_x_e4, dim=1)
...
if __name__ == '__main__':
    model = MISTNet()
    inputs = torch.randn((1, 3, 5, 384, 384))

...
from deepmist.models.singleframe.RDIAN.cbam import *
from deepmist.models.singleframe.RDIAN.direction import *
...
class RDIAN(nn.Module):
    def __init__(self):
        ...
        self.residual_block0 = self.make_layer(NewBlock, in_channels=32, num_blocks=1, kernel_size=1, padding=0,
                                               stride=1)
        self.residual_block1 = self.make_layer(NewBlock, in_channels=32, num_blocks=2, kernel_size=3, padding=1,
                                               stride=1)
        self.residual_block2 = self.make_layer(NewBlock, in_channels=32, num_blocks=2, kernel_size=5, padding=2,
                                               stride=1)
        self.residual_block3 = self.make_layer(NewBlock, in_channels=32, num_blocks=2, kernel_size=7, padding=3,
                                               stride=1)
    ...
    def forward(self, x):
        # Given groups=1, weight of size [1, 1, 5, 5], expected input[12, 3, 384, 384] to have 1 channels, but got 3 channels instead
        x = x[:, 0, :, :]
        x = x.unsqueeze(1)
...
if __name__ == '__main__':
    # model = RDIAN().cuda()
    # inputs = torch.randn((20, 1, 256, 256)).cuda()  # Params = 0.22M FLOPs = 74.37G

    model = RDIAN().cuda()
    inputs = torch.randn((1, 3, 384, 384)).cuda()  # Params = 0.22M FLOPs = 8.37G

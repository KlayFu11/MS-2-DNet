...
class PyramidMiM_enc(nn.Module):
    ...
    def __init__(self, configs=None, img_size=384, in_chans=3, num_classes=1, mlp_ratio=4., qkv_bias=False,
...
class MiM(nn.Module):
    def __init__(self, layer_blocks=[2] * 3, channels=[8, 16, 32, 64, 128]):
    ...
    def forward(self,
                x):  # the input is of size (b,3,512,512), the output is of size (b,1,512,512), where the num_class=1 in ISTD.
        _, _, hei, wid = x.shape
...
if __name__ == '__main__':
    model = MiM().cuda()
    inputs = torch.randn((1, 3, 384, 384)).cuda()  # Params = 1.18M FLOPs = 0.58G

...
from deepmist.models.singleframe.ISNet.my_functionals import GatedSpatialConv as gsc
from deepmist.models.singleframe.ISNet.network import Resnet
from torch.nn.parameter import Parameter
from deepmist.models.singleframe.ISNet.DCNv2.TTOA import TTOA
from deepmist.models.singleframe.ISNet.train_ISNet import Get_gradient_nopadding
...
class ISNet_ours(nn.Module):
    def __init__(self, layer_blocks=[4] * 3, channels=[8, 16, 32, 64], num_classes=1, use_edge_loss=True):
        super(ISNet_ours, self).__init__()
        self.use_edge_loss = use_edge_loss
        self.ISNet = ISNet(layer_blocks, channels, num_classes)
        self.grad = Get_gradient_nopadding()

    def forward(self, x):
        edge_in = self.grad(x)
        pred, edge_out = self.ISNet(x, edge_in)
...
if __name__ == '__main__':
    model = ISNet_ours().cuda()
    inputs = torch.randn((1, 3, 384, 384)).cuda()  # Params = 0.97M FLOPs = 68.92G

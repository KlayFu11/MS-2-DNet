...
class ASKCResNetFPN(nn.Module):
    def __init__(self, in_channels=3, layers=[4, 4, 4], channels=[8, 16, 32, 64], fuse_mode='AsymBi', act_dilation=16,
                 classes=1, tinyFlag=False, norm_layer=BatchNorm2d, groups=1, norm_kwargs=None, **kwargs):
...
if __name__ == '__main__':
    # model = ASKCResNetFPN(in_channels=1).cuda()
    # inputs = torch.randn((20, 1, 256, 256)).cuda()  # Params = 0.43M FLOPs = 7.56G

    model = ASKCResNetFPN(in_channels=3).cuda()
    inputs = torch.randn((1, 3, 384, 384)).cuda()  # Params = 0.43M FLOPs = 0.86G

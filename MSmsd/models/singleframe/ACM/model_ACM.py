...
class ASKCResUNet(nn.Module):
    def __init__(self, in_channels=3, layers=[3, 3, 3], channels=[8, 16, 32, 64], fuse_mode='AsymBi', tiny=False,
                 classes=1, norm_layer=BatchNorm2d, groups=1, norm_kwargs=None, **kwargs):
...
if __name__ == '__main__':
    # model = ASKCResUNet(in_channels=1).cuda()
    # inputs = torch.randn((20, 1, 256, 256)).cuda()  # Params = 0.4M FLOPs = 8.05G

    model = ASKCResUNet(in_channels=3).cuda()
    inputs = torch.randn((1, 3, 384, 384)).cuda()  # Params = 0.4M FLOPs = 0.91G

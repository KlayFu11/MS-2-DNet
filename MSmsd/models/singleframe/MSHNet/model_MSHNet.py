...
from deepmist.models.singleframe.MSHNet.APConv import PConv
...
class ResNet(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1, use_pconv=False):
        ...
        if self.use_pconv:
            self.conv1 = PConv(in_channels, out_channels, 4, stride)
            self.conv2 = PConv(out_channels, out_channels, 3, 1)
        else:
...
class MSHNet(nn.Module):
    def __init__(self, input_channels=3, block=ResNet, use_pconv=False):
        super().__init__()
        ...
        self.encoder_0 = self._make_layer(param_channels[0], param_channels[0], block, use_pconv=use_pconv)
    ...
    def _make_layer(self, in_channels, out_channels, block, block_num=1, use_pconv=False):
        layer = []
        layer.append(block(in_channels, out_channels, use_pconv=use_pconv))
...
if __name__ == '__main__':
    model = MSHNet(use_pconv=False).cuda()
    inputs = torch.randn((1, 3, 384, 384)).cuda()  # Params = 4.07M FLOPs = 13.76G

    # model = MSHNet(use_pconv=True).cuda()
    # inputs = torch.randn((1, 3, 384, 384)).cuda()  # Params = 4.06M FLOPs = 13.66G

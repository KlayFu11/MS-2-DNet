...
encoder_names = ['encoder_1', 'encoder_2', 'encoder_3', 'encoder_4']

for i, (feat, encoder_name) in enumerate(zip(features, encoder_names)):
    print(f"\n处理特征图 (encoder_{i+1}): {feat.shape}")
    
    # 保存完整通道的原始特征图张量
    save_feature_map_tensor(feat, os.path.join(save_dir, f'98_{encoder_name}_features.pt'))  # 保存为PyTorch张量
    save_feature_map_tensor(feat, os.path.join(save_dir, f'98_{encoder_name}_features.npy'))  # 保存为NumPy数组

    # 生成无标注的纯图像
    visualize_feature_map_simple(feat, os.path.join(save_dir, f'98_{encoder_name}_visualization.png'))
print("\n" + "="*60)
print("PNG格式特征图大小压缩原因解释:")
print("="*60)
print("1. 维度压缩: 原始特征图是多通道的4D张量，")
